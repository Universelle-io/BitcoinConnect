...
function marketDataAdapter(data) {
    return data.map((item) => {
        const icon = item.id === "bitcoin" ? index_js_1.bitcoinImg : index_js_1.counterpartyImg;
        return {
            ...item,
            icon
        };
...
exports.openbook = {
    ...
    getUTXOS: async ({ address }) => {
        const endpoint = new URL(`${(0, config_js_1.getConfig)().OPENBOOK.ENDPOINT}/api/v1/xcp/utxos/${address}`);
        const data = await fetch(endpoint);
    ...
    getAtomicSales: async ({ limit, page }) => {
        const endpoint = new URL(`${(0, config_js_1.getConfig)().OPENBOOK.ENDPOINT}/api/v1/atomic-swaps`);
    ...
    getAtomicSalesByAsset: async ({ asset, limit, page }) => {
        const endpoint = new URL(`${(0, config_js_1.getConfig)().OPENBOOK.ENDPOINT}/api/v1/atomic-swaps/asset/${asset}`);
        endpoint.searchParams.set("limit", limit?.toString() || "100");
        endpoint.searchParams.set("page", page?.toString() || "1");
        const data = await fetch(endpoint);
        const atomic_swaps = await data.json();
    ...
    getAtomicSalesByAddress: async ({ address, limit, page }) => {
        const endpoint = new URL(`${(0, config_js_1.getConfig)().OPENBOOK.ENDPOINT}/api/v1/atomic-swaps/address/${address}`);
        endpoint.searchParams.set("limit", limit?.toString() || "100");
        endpoint.searchParams.set("page", page?.toString() || "1");
        const data = await fetch(endpoint);
        const atomic_swaps = await data.json();
    ...
    getAtomicSaleByTxId: async ({ txid }) => {
        const endpoint = new URL(`${(0, config_js_1.getConfig)().OPENBOOK.ENDPOINT}/api/v1/atomic-swaps/tx/${txid}`);
        const data = await fetch(endpoint);
    ...
    getAtomicSwapOrders: async ({ limit, page }) => {
        const endpoint = new URL(`${(0, config_js_1.getConfig)().OPENBOOK.ENDPOINT}/api/v1/orders`);
    ...
    getAtomicSwapOrdersByAsset: async ({ asset }) => {
        const endpoint = new URL(`${(0, config_js_1.getConfig)().OPENBOOK.ENDPOINT}/api/v1/orders/asset/${asset}`);
        const data = await fetch(endpoint);
        const atomic_swaps = await data.json();
        return atomic_swaps;
    },
    ...
    cancelAtomicSwap: async ({ id, feeRate }) => {
        const endpoint = new URL(`${(0, config_js_1.getConfig)().OPENBOOK.ENDPOINT}/api/v1/orders/cancel`);
        const data = await fetch(endpoint, {
            method: "POST",
            headers: {
    ...
    getPsbtForListOrder: async ({ utxo, seller, price }) => {
        const endpoint = new URL(`${(0, config_js_1.getConfig)().OPENBOOK.ENDPOINT}/api/v1/orders/list/sign`);
        const data = await fetch(endpoint, {
            method: "POST",
            headers: {
                "Content-Type": "application/json"
    ...
    getPsbtForSubmitOrderOnchain: async ({ utxo, seller, price, feeRate, psbt }) => {
        const endpoint = new URL(`${(0, config_js_1.getConfig)().OPENBOOK.ENDPOINT}/api/v1/orders/list/submit`);
        const data = await fetch(endpoint, {
            method: "POST",
            headers: {
                "Content-Type": "application/json"
    ...
    getBuyOrderPSBT: async ({ id, buyer, feeRate, serviceFee = [] }) => {
        const endpoint = new URL(`${(0, config_js_1.getConfig)().OPENBOOK.ENDPOINT}/api/v1/orders/buy`);
        const data = await fetch(endpoint, {
            method: "POST",
            headers: {
                "Content-Type": "application/json"
    ...
    utils: {
        ...
        getCIP25JSON: async ({ cip25Url }) => {
            try {
                const endpoint = new URL(`${(0, config_js_1.getConfig)().OPENBOOK.ENDPOINT}/api/v1/utils/cip25?url=${cip25Url}`);
                const data = await fetch(endpoint);
        ...
        getMempoolFees: async () => {
            try {
                const endpoint = new URL(`${(0, config_js_1.getConfig)().OPENBOOK.ENDPOINT}/api/v1/utils/mempool-fees`);
                const data = await fetch(endpoint);
